...
class LifecycleManager:
    ...
    async def transition_to(self, new_state: BackendLifecycleState, error_info: str | None = None) -> None:
        ...
        async with self._lock:
            ...
            logger.debug(
                f"{self._backend_name}: {old_state.value} -> {new_state.value}"
            )
    ...
    async def cleanup(self) -> None:
        ...
        logger.debug(f"{self._backend_name}: Running {len(self._cleanup_callbacks)} cleanup callbacks")
