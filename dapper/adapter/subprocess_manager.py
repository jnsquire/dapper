...
class SubprocessManager:
    ...
    def _patch_subprocess(self) -> None:
        ...
        def patched_popen_init(self_popen, args, **kwargs):
            ...
            if manager._config.auto_attach and manager._is_python_command(args_list):
                port = manager._allocate_port()
                new_args = manager._build_launcher_args(args_list, port)
                ...
                child_info = ChildProcessInfo(
                    pid=self_popen.pid,
                    name=os.path.basename(args_list[-1]) if args_list else 'unknown',
                    ipc_port=port,
                    command=args_list,
                    cwd=kwargs.get('cwd'),
                    is_python=True,
    ...
    def _unpatch_subprocess(self) -> None:
        ...
        if self._original_popen is not None:
            subprocess.Popen.__init__ = self._original_popen  # type: ignore
            self._original_popen = None
