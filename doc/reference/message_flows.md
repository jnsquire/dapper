...
- Add rendered SVGs of these diagrams (pre-rendered using `mmdc`) and commit them alongside the markdown.
- Add a short script `scripts/render-mermaid.ps1` that runs mermaid-cli over `.mmd` files and outputs `.svg` into `doc/images`.
